...
class ACTION_SLOT(nn.Module):
    def __init__(self, args, num_ego_class, num_actor_class, num_slots=64, box=False, videomae=None):

        # En models/action_slot.py, dentro de __init__:
        if args.backbone == 'r50':
            if args.dataset == 'taco':
                self.resolution = (7, 7)         # Asegúrate que sea 7x7
                self.resolution3d = (args.seq_len, 7, 7) # Ajusta esto
        ...
        if args.backbone == 'r50':
            # self.resnet = r50.R50(base_channels=24) # <------ CAMBIO
            self.resnet = r50.R50()
            self.in_c = 2048
            if args.dataset == 'taco':
                # --- CORRECCIÓN AQUÍ ---
                # El checkpoint fue entrenado con estos valores, no los cambies
                self.resolution = (8, 24) 
                self.resolution3d = (16, 8, 24) 
                self.num_slots = 64 # Esto hará que num_slots+1 (bg_slot) sea 65
                # -----------------------
